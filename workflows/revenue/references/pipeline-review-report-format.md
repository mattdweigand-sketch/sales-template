...
Calendar start/end times prove scheduling, not attendance or outcomes. Never describe an upcoming or ongoing event as held. If the only evidence is a calendar entry, say `scheduled` or `elapsed, attendance unverified`. A Task receipt supports its title, verified direction, activity date, and contact only, not a summary of the email body.
...
The coverage header summarizes the actual successful coverage check, not an assumption. On incomplete coverage, replace it with `Coverage incomplete · <specific missing checks>` and withhold affected proposals. Keep the full checker receipts in the active run, not the report. Extended review uses its actual mail and Calendar coverage instead of the daily inbox claim.
