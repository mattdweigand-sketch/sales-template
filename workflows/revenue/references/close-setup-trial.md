...
1. Confirm `policy.close.setup_trial.enabled`, adapter capability, exact payload
   mapping, success fields and timeout. The example leaves this disabled. Without
   a configured flow, prepare the operations handoff and hold the close.
...
3. Snapshot every field in `restore_after` and every paid term the actual payload
   will overwrite, including subscription type, billing interval, start,
   deactivation and trial end dates where applicable. Later B proposals restore
   the signed value, or the exact pre-A0 value when the agreement is silent.
...
organization ID within the configured bound. A reset trigger or sync timestamp
proves an attempt, not success. Require the configured successful status, expected
organization/subscription IDs and no error.
